...
Сервер зберігає дані в таблицю `deye_log` і показує їх на дашборді. **Споживання за місяць** (кВт·год) обчислюється інтеграцією `load_power_w` по часу (метод трапецій) на основі існуючих записів — додаткові регістри Modbus не потрібні.
